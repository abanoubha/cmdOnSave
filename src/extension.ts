...
export function activate(context: vscode.ExtensionContext): void {
	...
	var extension = new CmdOnSaveExtension(context);
	extension.showOutputMessage();

	vscode.workspace.onDidChangeConfiguration(() => {
		let disposeStatus = extension.showStatusMessage('Cmd On Save: Reloading config.');
...
class CmdOnSaveExtension {
	...
	constructor(context: vscode.ExtensionContext) {
		...
		this._outputChannel = vscode.window.createOutputChannel('Cmd On Save');
		this.loadConfig();
	}
	...
	private _runCommands(
		...
		if (commands.length) {
			...
			// NOTE: marks the end of commands starting.
			// There could still be asyc commands running.
			this.showStatusMessage('Cmd On Save done.');
		}
	}

	private _getExecOption(document: vscode.TextDocument): { shell: string, cwd: string } {
		var str = this._getWorkspaceFolderPath(document.uri);

		if (str === null) {
			console.log("current working directory (cwd) is NULL");
		}

		return {
			shell: this.shell,
			cwd: str?.toString.name!,
		};
	}
	...
	public loadConfig(): void {
		// this._config = <IConfig><any>vscode.workspace.getConfiguration('abanoubha.cmdonsave');
		this._config = <IConfig><any>vscode.workspace.findFiles('**​/cmdOnSave.json', '**​/node_modules/**', 1)
